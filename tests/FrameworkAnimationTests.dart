class FrameworkAnimationTests extends TestGroupBase {
  ...
  void setPropertyTransition(){
    ...
    Expect.equals('background 1s linear 0s', Dom.getXPCSS(b.rawElement, 'transition'));


    //add another one
    FrameworkAnimation.setPropertyTransition(b.borderColorProperty, new PropertyTransition(1, TransitionTiming.linear));

    Expect.equals('background 1s linear 0s, border 1s linear 0s', Dom.getXPCSS(b.rawElement, 'transition'));


    //replace value
    FrameworkAnimation.setPropertyTransition(b.backgroundProperty, new PropertyTransition(3, TransitionTiming.easeIn));

    Expect.equals('background 3s ease-in 0s, border 1s linear 0s', Dom.getXPCSS(b.rawElement, 'transition'));
  }
  ...
  void removePropertyTransition(){
    ...
    Expect.equals('border 1s linear 0s', Dom.getXPCSS(b.rawElement, 'transition'));

    FrameworkAnimation.clearPropertyTransition(b.borderColorProperty);

    Expect.isNull(Dom.getXPCSS(b.rawElement, 'transition'));
